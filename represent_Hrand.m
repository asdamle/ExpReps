function [nodes, weights, xloc] = represent_Hrand(f,acc,l)
% Anil Damle
...
% nrand = 10*ke;
% r = rand(1,nrand);
% theta = 2*pi*rand(1,nrand);
% root_init = sqrt(r).*(cos(theta) + 1i*sin(theta));
% rxN = newton_vector(flip(x(:).'),polyder(flip(x(:).')),root_init,acc);
% rx = rxN(abs(rxN)<1);


%find all of them and filter
rx = roots(flip(x(:).'));
rx = rx(abs(rx)<1);
